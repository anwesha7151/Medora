...
const path = "src/services/medicines.ts";
const content = `import { demoMedicines, demoPharmacies } from "@/data/demo-catalog";
...
const executeWithFallback = async <T>(operation: (provider: ReturnType<typeof getProvider>) => Promise<T>): Promise<T> => {
...
export const getOffers = async (medicineIds: string[]): Promise<OfferRow[]> => {
  ...
  for (const listing of listings) {
    ...
    if (listing.provenance.source === "Medora Demo Data") {
      ...
      medicine = await provider.getMedicine(listing.medicineId).catch(() => undefined);
      pharmacy = await provider.getPharmacy(listing.pharmacyId).catch(() => undefined);
    }
